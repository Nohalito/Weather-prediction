...
% Coloring :
\definecolor{Section}{RGB}{47, 89, 229}
...
consumption. Early studies such as Bessec and Fouquau (2008) show that temperature energy
relationships are inherently non-linear, reflecting asymmetric heating and cooling needs. More recent
contributions, including Auffhammer et al. (2017) and Deschênes and Greenstone (2011),
...
variables have been widely employed for short-term energy demand forecasting (see Hong et al.,
...
meteorological variables, particularly temperature, due to their strong seasonal structure (Box et al.,
...
and provides Meteorological data. This dataset contains daily observations collected by weather stations
...
of more sophisticated models can be evaluated. \\
\textit{SARIMA model} \\
...
In the time-series framework, the SARIMA model is estimated using Maximum Likelihood Estimation
...
\begin{center}
    $\textbf{E}(\epsilon_t \mid F_{t-1}) = 0$
\end{center}
...
MLE estimation also allows the direct derivation of point forecasts for the temperature series $TM_t$, as well
...
to produce optimal forecasts conditional on past information. SARIMA models belong to the class
of stochastic dynamic models, in which the observed variable depends on its past values,
...
directly on the conditional distribution of the series given past information. The likelihood function
...
\begin{table}[htbp]
\centering
\caption{SARIMA Model Performance}
\label{tab:rolling_models}
\begin{tabular}{lcc}
\hline
Model & RMSE & MAE \\
\hline
ETS (rolling)              & 1.32 & 1.06 \\
SARIMA (rolling)           & 1.39 & 1.09 \\
Naïf saisonnier (rolling)  & 1.98 & 1.62 \\
\hline
\end{tabular}
\end{table}

The estimation results \textbf{\cite{SARIMA}} highlight the importance of seasonal dynamics. Seasonal autoregressive and
...
cross-validation must take into account the temporal structure of the data. A 
classic K-fold cross-validation procedure is not appropriate, as it would violate 
the chronological order of the observations and introduce information leakage.

A temporal cross-validation strategy was therefore adopted, based on a rolling 
window approach. At each iteration, the model is estimated over an initial 
learning period and then used to produce forecasts for the following period. 
The learning window is then moved forward in time, and the operation is 
repeated.

This method makes it possible to evaluate the stability of predictive 
performance over time and to verify the model's ability to adapt to new 
observations. Forecast errors are calculated at each iteration and aggregated 
over the entire validation period.

The results show that the SARIMA model performs relatively consistently 
across the different validation windows, with RMSE and MAE values consistent 
with those obtained in the initial out-of-sample evaluation. The naive 
seasonal model also performs consistently, but with systematically higher 
errors.

These results confirm that the performance gain observed for the SARIMA model 
is not due to chance or a particular sample split, but reflects a robust 
improvement in forecast quality.

Temporal cross-validation thus validates the choice of the SARIMA model as 
the final forecasting method for average temperature.

\begin{figure}[h]
\centering
\includegraphics[width=1\textwidth]{../results/figures/Diallo_S2_Q11_RMSE_comparison.png}
\caption{RMSE}
\label{fig:CV}
\end{figure}

Temporal cross-validation using rolling windows allows us to evaluate the 
robustness of the predictive performance of different average temperature 
forecasting models. This approach respects the chronological order of 
observations and is a method suited to time series.

The results show that the naive seasonal model performs the worst, with an 
RMSE of approximately 1.98 and an MAE of approximately 1.62. Although this 
model manages to capture the overall seasonal structure of the series, it does 
not take into account short-term fluctuations and interannual variations, 
which limits its predictive capacity.

The SARIMA model significantly improves performance compared to the naive 
benchmark. The RMSE obtained is approximately 1.41 and the MAE is approximately 
1.09, indicating a substantial reduction in forecast errors. This improvement 
reflects the model's ability to exploit not only annual seasonality but also 
the short-term temporal dependencies present in the series.

The exponential smoothing (ETS) model performs best among the methods 
compared, with an RMSE of approximately 1.32 and an MAE of approximately 1.06. 
The results suggest that explicitly decomposing the series into level and 
seasonality components effectively captures the dynamics of the average 
temperature over the validation period.

Analysis of the forecast error graph for the SARIMA model shows that errors 
are generally centered around zero, with no apparent systematic drift. The 
fluctuations observed remain moderate in amplitude, indicating good forecast 
stability over time, despite a few isolated episodes of higher errors.

Overall, time cross-validation confirms that the SARIMA and ETS models offer 
robust predictive performance that is superior to that of the naive seasonal 
model. The slight advantage observed for the ETS model suggests that explicit 
modeling of seasonality and level is particularly appropriate in this context. 
However, the similar performance of the ETS and SARIMA models indicates that 
both approaches are relevant choices for forecasting average temperature.

These results confirm that the conclusions drawn during the initial out-of-sample 
evaluation do not depend on a particular sample split, but reflect a robust 
improvement in forecast quality.
...
The energy data used in this part of the project come from Open Data Réseaux Énergies (ODRÉ) \cite{ODRE}. This source
...
The dependent variable in this study is total
...
France. They include average daily temperature (TM, in Celsius), average humidity (UM, in \%),
...
\\
\textbf{Generalized additive model} authorize non-linear relation between energy consumption et some weather features :\par
\begin{center}
    $Y_i = \alpha + \sum_{j}^{}f_j(X_{ij}) + \epsilon_i$
\end{center}
...
This subsection presents the estimation results \cite{OLS} for daily energy consumption using a cross-sectional
...
\begin{tabular}{lcc}
\hline
Model & RMSE$_{\text{test}}$ & MAE$_{\text{test}}$ \\
\hline
OLS   & 29133.79 & 23529.47 \\
Ridge & 29135.15 & 23538.39 \\
Lasso & 29142.80 & 23551.10 \\
\hline
\end{tabular}
\end{table}
...
% ------------------------ Cross-validation ------------------------
\subsection{Cross-validation}
To assess the robustness of the predictive performance of the models estimated for 
energy consumption, a cross-validation procedure was implemented. The objective 
is to verify that the results obtained do not depend on a particular division of 
the sample and that they generalize correctly to new observations.

A K-fold cross-validation strategy is used. The sample is divided 
into several subsamples, and at each iteration, one subsample is reserved for 
prediction while the others are used for model estimation. The prediction errors are 
then aggregated across all folds.

This procedure is applied to linear models (OLS, Ridge, Lasso) as well as to the nonlinear GAM model. 
The comparison focuses on the average RMSE and MAE, as well as their variability 
between folds.

The results show that the performance of the models is relatively stable from one fold to 
another, indicating an absence of overfitting. The GAM model performs 
comparably, if not slightly better, while maintaining good robustness. This cross-validation 
confirms that the choice of model does not depend on a particular data split, but 
reflects a satisfactory generalization ability.
...
Model & RMSE & MAE \\
\hline
GAM (pyGAM) & 28761.99 & 23249.28 \\
RidgeCV & 36164.32 & 29744.00 \\
OLS & 36165.02 & 29738.86 \\
LassoCV & 36165.46 & 29753.03 \\
\hline
\end{tabular}
\end{table}

K-fold cross-validation applied to the GAM model allows us to evaluate the 
robustness of its predictive performance on different partitions of the 
sample. Five folds were used, each successively acting as a test sample.

The results show that the RMSE values obtained on the different folds are 
relatively close, ranging from approximately 27,900 to 29,000 MW. Similarly, 
the MAE values vary within a limited range, from approximately 22,500 to 
23,950 MW. This low dispersion of errors between folds indicates that the 
model's performance does not depend on a particular sample split.

The average RMSE from cross-validation is approximately 28,741 MW, with a 
standard deviation of approximately 604 MW. The average MAE is approximately 
23,225 MW, with a standard deviation of approximately 593 MW. These relatively 
low standard deviations compared to the average level of errors suggest good 
stability in the predictive performance of the GAM model.

These results confirm that the GAM model does not suffer from overfitting and 
that it generalizes correctly to new observations. They are consistent with 
the out-of-sample performance observed previously and validate the choice of 
this model as a tool for forecasting energy consumption.

In conclusion, cross-validation reinforces the credibility of the chosen model 
by showing that its predictive capacity is robust and stable, while benefiting 
from the flexibility offered by nonlinear modeling of the relationships between 
energy consumption and meteorological variables.
...
\begin{figure}[h]
\centering
\includegraphics[width=1\textwidth]{../results/figures/Diallo_S1_Q12_selected_predictions_scatter.png}
...
\bibitem{OLS}
\verbatiminput{../results/logs/Diallo_S1_Q8_OLS_summary.txt}

\bibitem{SARIMA}
\verbatiminput{../results/logs/Diallo_S2_Q9_SARIMA_summary.txt}
